#!/usr/bin/env python3
"""
v10 - CORS & IAM Perimeter Dissolution scanner

Scans source, config, and IaC files for overly permissive network/identity
boundaries:
  - Wildcard CORS origins and credential echoing
  - cors() middleware applied without options
  - IAM Action/Resource wildcards (AWS JSON, Terraform, Serverless, K8s RBAC)
  - Outbound HTTP calls with user-controlled URL (SSRF paths to IMDS)
  - IMDS / metadata endpoint references
  - Supabase service-role keys, NEXT_PUBLIC_ secrets, Firebase Admin in client
  - Over-provisioned AWS keys in application code
...
# --------------------------------------------------------------------------
# Minimal YAML reader — same subset used across v1-v9
# --------------------------------------------------------------------------
...
def main():
    ap = argparse.ArgumentParser(description="V10 CORS & IAM Perimeter Dissolution scanner")
    ...
    if args.json:
        ...
        print(f"\n=== V10 CORS & IAM Perimeter Dissolution Scan :: {args.target} ===")
