<#
.SYNOPSIS
    V10 CORS & IAM Perimeter Dissolution scanner (PowerShell edition).
    Scans source, config, and IaC files for overly permissive network/identity
    boundaries: wildcard CORS, IAM action/resource wildcards, user-controlled
    outbound HTTP (SSRF), IMDS references, and over-provisioned service keys.
...
# -----------------------------------------------------------------------
# Parse config.yml — purpose-built for the v10 config shape.
# -----------------------------------------------------------------------
function Read-V10Config {
    param([string]$Path)
...
# ---- load config -------------------------------------------------------
$cfg        = Read-V10Config -Path $cfgPath
...
if ($Json) {
    ...
    Write-Host "`n=== V10 CORS & IAM Perimeter Dissolution Scan :: $Target ==="
