...
Get-ChildItem -Path $Target -Recurse -File -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    $rel = if ($abs.StartsWith($base)) { $abs.Substring($base.Length).TrimStart("\").Replace("\", "/") } else { $abs }
        ...
        if ($eco -eq "npm") {
            ...
            foreach ($ln in $lines) {
                ...
                if ($ln -and -not $ln.StartsWith("-")) {
                    if ($ln -match '^([A-Za-z0-9_\.\-]+)') { $pkgs += $Matches[1] }
                }
            }
        } elseif ($eco -eq "go") {
...
# ---- output ----
if ($Json) {
    @{ count = $findings.Count; findings = $findings } | ConvertTo-Json -Depth 4
} else {
