<#
.SYNOPSIS
    V7 AI Default Credentials scanner (PowerShell edition).
    Scans infrastructure and config files for predictable boilerplate credentials:
    weak Docker Compose env pairs, exposed DB ports, admin seed accounts,
    SSH password auth, shell hardcoded users, RabbitMQ/Keycloak/Redis defaults,
    and Kubernetes secrets with base64-encoded weak passwords.
...
# -----------------------------------------------------------------------
# Parse config.yml — purpose-built for the v7 config shape.
# Reads: exclude_patterns, source_extensions (lists),
#        patterns (list of maps: name/regex/category/severity/note).
# -----------------------------------------------------------------------
function Read-V7Config {
    param([string]$Path)
    ...
    foreach ($raw in $lines) {
        ...
        # scalar list item
        if ($trimmed.StartsWith("- ") -and $indent -eq 2 -and $section -in @("exclude_patterns","source_extensions","weak_passwords","sensitive_ports","weak_b64")) {
...
# ---- load config -------------------------------------------------------
$cfg        = Read-V7Config -Path $cfgPath
...
if ($Json) {
    ...
    Write-Host "`n=== V7 AI Default Credentials Scan :: $Target ==="
