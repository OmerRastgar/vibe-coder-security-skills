#!/usr/bin/env python3
"""
v7 - AI Default Credentials scanner

Scans infrastructure and config files for predictable, boilerplate credentials:
  - Weak passwords in Docker Compose env blocks (Postgres, MySQL, Mongo)
  - Globally published database / service ports
  - Admin dashboard defaults (Grafana, pgAdmin)
  - Seed records creating admin accounts without forced password reset
  - SSH PasswordAuthentication enabled
  - Hardcoded system-user credentials in shell scripts
  - RabbitMQ guest/guest and Keycloak admin defaults
  - Redis with no auth or weak password
  - Kubernetes secrets with base64-encoded weak passwords
...
# --------------------------------------------------------------------------
# Minimal YAML reader — same subset used across v1-v6
# --------------------------------------------------------------------------
...
def main():
    ap = argparse.ArgumentParser(description="V7 AI Default Credentials scanner")
    ...
    if args.json:
        ...
        print(f"\n=== V7 AI Default Credentials Scan :: {args.target} ===")
