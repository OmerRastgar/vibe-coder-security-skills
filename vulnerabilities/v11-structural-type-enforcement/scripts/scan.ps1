<#
.SYNOPSIS
    V11 Structural Type Enforcement scanner (PowerShell edition).
    Scans source files for loose typing and mass assignment flaws:
    req.body spread, 'as any' casts, Python dict unpack into ORM,
    Pydantic extra='allow', Django fields='__all__', Go untyped unmarshal,
    and blind AutoMapper/BeanUtils usage.
...
# -----------------------------------------------------------------------
# Parse config.yml — purpose-built for the v11 config shape.
# -----------------------------------------------------------------------
function Read-V11Config {
    param([string]$Path)
...
# ---- load config -------------------------------------------------------
$cfg        = Read-V11Config -Path $cfgPath
...
if ($Json) {
    ...
    Write-Host "`n=== V11 Structural Type Enforcement Scan :: $Target ==="
