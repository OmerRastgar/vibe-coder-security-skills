<#
.SYNOPSIS
    V9 Denial of Wallet & Rate-Limiting scanner (PowerShell edition).
    Scans backend source for unthrottled high-cost paths: unguarded AI API
    calls, synchronous compute-heavy handlers, auth routes without rate
    limiters, and unbounded DB queries.
...
# -----------------------------------------------------------------------
# Parse config.yml — purpose-built for the v9 config shape.
# -----------------------------------------------------------------------
function Read-V9Config {
    param([string]$Path)
...
# ---- load config -------------------------------------------------------
$cfg        = Read-V9Config -Path $cfgPath
...
if ($Json) {
    ...
    Write-Host "`n=== V9 Denial of Wallet Scan :: $Target ==="
