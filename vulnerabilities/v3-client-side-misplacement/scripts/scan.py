#!/usr/bin/env python3
"""
v3 - Client-Side Security Misplacement scanner

Scans frontend source files for patterns that indicate security logic
placed only in the browser (route guards, JWT decoding, client-side math,
feature-flag gating, HTML-only input limits, etc.).
...
# --------------------------------------------------------------------------
# Minimal YAML reader (no PyYAML dependency) — same subset as v1/v2
# --------------------------------------------------------------------------
...
def main():
    ap = argparse.ArgumentParser(description="V3 Client-Side Misplacement scanner")
    ...
    if args.json:
        ...
        print(f"\n=== V3 Client-Side Misplacement Scan :: {args.target} ===")
