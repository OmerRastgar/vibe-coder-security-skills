<#
.SYNOPSIS
    V5 Missing Input Validation scanner (PowerShell edition).
    Scans backend source files for shallow or absent input validation:
    mass assignment, missing schema validation, extension-only file checks,
    unbounded upload sizes, pagination param abuse, missing webhook HMAC,
    timing-unsafe signature comparisons, and header log injection.
...
# ---- load config -------------------------------------------------------
$cfg        = Read-V5Config -Path $cfgPath
...
if ($Json) {
    ...
    Write-Host "`n=== V5 Missing Input Validation Scan :: $Target ==="
