<#
.SYNOPSIS
    V8 Flawed Object Authorization scanner (PowerShell edition).
    Scans backend source for BOLA/IDOR, mass assignment, sequential ID
...
# -----------------------------------------------------------------------
# Parse config.yml — purpose-built for the v8 config shape.
# -----------------------------------------------------------------------
function Read-V8Config {
    param([string]$Path)
...
# ---- load config -------------------------------------------------------
$cfg        = Read-V8Config -Path $cfgPath
...
if ($Json) {
    ...
    Write-Host "`n=== V8 Flawed Object Authorization Scan :: $Target ==="
