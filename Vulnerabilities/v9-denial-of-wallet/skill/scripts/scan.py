#!/usr/bin/env python3
"""
v9 - Denial of Wallet & Rate-Limiting scanner

Scans backend source files for unthrottled, high-cost paths:
  - Unguarded OpenAI / Anthropic / HuggingFace / Cohere API calls
  - LLM completions without a max_tokens cap
  - AI-related routes with no visible rate limiter
  - Synchronous PDF / image / archive processing in request handlers
  - Login / register / OTP-resend routes without rate limiting
  - Unbounded LIKE wildcard queries, MongoDB $regex, SELECT *
  - ElasticSearch queries without a size cap
...
# --------------------------------------------------------------------------
# Minimal YAML reader — same subset used across v1-v8
# --------------------------------------------------------------------------
...
def main():
    ap = argparse.ArgumentParser(description="V9 Denial of Wallet scanner")
    ...
    if args.json:
        ...
        print(f"\n=== V9 Denial of Wallet Scan :: {args.target} ===")
